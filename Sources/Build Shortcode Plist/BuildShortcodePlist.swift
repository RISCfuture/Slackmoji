...
struct BuildShortcodePlist: AsyncParsableCommand {
    @Option(completion: .file(extensions: ["plist"]), transform: { .init(filePath: $0, directoryHint: .notDirectory) })
    var emojiToSlackmojiFile = URL(filePath: "Sources/Slackmoji/Resources/EmojiToSlackmoji.plist", directoryHint: .notDirectory)

    @Option(completion: .file(extensions: ["plist"]), transform: { .init(filePath: $0, directoryHint: .notDirectory) })
    var slackmojiToEmojiFile = URL(filePath: "Sources/Slackmoji/Resources/SlackmojiToEmoji.plist", directoryHint: .notDirectory)
